...
import qualified Pure.Data.Txt.Trie as T
import Pure.Data.View (View(HTMLView,KHTMLView,SVGView,KSVGView,TextView,tag))
...
tags = nub . collect
  where
    nub xs = List.foldr go (const []) xs mempty
      where
        go key cont hm 
          | T.member key hm = cont hm
          | otherwise       = key : cont (T.insert key () hm)  
